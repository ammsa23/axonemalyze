...
#     """
#     Loads the coordinates from a CSV file.

#     Parameters:
#     file_path: str
#         Path to the CSV file containing x, y, z columns.

#     Returns:
#     vertices: np.array
#         Numpy array of shape (n, 3) containing 3D coordinates.
#     """
#     coords = pd.read_csv(file_path)
#     vertices = coords[['x', 'y', 'z']].values
#     return vertices
...
def order_circle_coordinates(current_coord, all_coords):
    ...
    for coords in all_coords:
        circle_coords.append(coords[np.argmin(cdist(current_coord.reshape(1, 3), coords))])
    ...
    while len(circle_coords) > 0:
        ...
        if len(circle_coords) == 0:
            break
        old = current
        current = circle_coords[np.argmin(cdist(np.array(old).reshape(1, 3), np.array(circle_coords).reshape(-1, 3)))]
        idx = np.argmin(cdist(np.array(old).reshape(1, 3), np.array(circle_coords).reshape(-1, 3)))

    return ordered_circle_coords

def estimate_circularity():

    parser = ArgumentParser(description="Estimate the circularity of an axoneme/partial axoneme by using angles between edges.")
    parser.add_argument("input_directory", help="Input directory containing .csv files with invidually segmented axonemes; this directory is outputted by `segment_axonemes.py`.")
    ...
    for i,csv in enumerate(csvs):
        ...
        df = pd.read_csv(csv)
        all_coords = [df.loc[df.label == label][['x', 'y', 'z']].values for label in sorted(np.unique(df.label.values))]
        ...
        for coord in min_coords:
            ...
            if len(ordered_circle_coords) >= 5 and len(ordered_circle_coords) < 9:
                ...
                for k in np.arange(ordered.shape[0]-3):
                    current_angle = angle_between_points(ordered[k], ordered[k+1], ordered[k+2])
        ...
        if np.unique(df.label.values).shape[0] >= 5:
            print(f'There are {np.unique(df.label.values).shape[0]} doublets in {os.path.split(csv)[-1]}!\nCircularity: {score_function(np.array(maes).mean())}')
